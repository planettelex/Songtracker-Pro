...
{
    [TestClass]
    public class ListRecordLabelDocumentsTests : TestsBase
    {
        [TestMethod]
        public void TaskSuccessTest()
        {
            ...
            var testItem1 = TestsModel.Document(testRecordLabel);
            ...
            var testItem2 = TestsModel.Document(testRecordLabel);
            ...
            var task = new ListRecordLabelDocuments(DbContext);
            var result = task.DoTask(testRecordLabel);

            Assert.IsTrue(result.Success);
            Assert.IsNull(result.Exception);

            var recordLabelDocuments = result.Data;
            Assert.IsNotNull(recordLabelDocuments);
            Assert.IsTrue(recordLabelDocuments.Count >= 2);

            var storageItem1 = recordLabelDocuments.SingleOrDefault(dm => dm.Uuid == storageItem1Id);
            ...
            var storageItem2 = recordLabelDocuments.SingleOrDefault(dm => dm.Uuid == storageItem2Id);
        ...
        {
            var task = new ListRecordLabelDocuments(EmptyDbContext);
