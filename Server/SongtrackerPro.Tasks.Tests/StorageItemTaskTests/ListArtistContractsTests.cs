...
{
    [TestClass]
    public class ListArtistContractsTests : TestsBase
    {
        [TestMethod]
        public void TaskSuccessTest()
        {
            ...
            var testItem1 = TestsModel.Contract(testArtist);
            ...
            var testItem2 = TestsModel.Contract(testArtist);
            ...
            var task = new ListArtistContracts(DbContext);
            var result = task.DoTask(testArtist);

            Assert.IsTrue(result.Success);
            Assert.IsNull(result.Exception);

            var artistContracts = result.Data;
            Assert.IsNotNull(artistContracts);
            Assert.IsTrue(artistContracts.Count >= 2);

            var storageItem1 = artistContracts.SingleOrDefault(dm => dm.Uuid == storageItem1Id);
            ...
            var storageItem2 = artistContracts.SingleOrDefault(dm => dm.Uuid == storageItem2Id);
        ...
        {
            var task = new ListArtistContracts(EmptyDbContext);
