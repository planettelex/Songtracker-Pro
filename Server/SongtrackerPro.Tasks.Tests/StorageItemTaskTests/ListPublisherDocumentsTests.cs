...
{
    [TestClass]
    public class ListPublisherDocumentsTests : TestsBase
    {
        [TestMethod]
        public void TaskSuccessTest()
        {
            ...
            var testItem1 = TestsModel.Document(testPublisher);
            ...
            var testItem2 = TestsModel.Document(testPublisher);
            ...
            var task = new ListPublisherDocuments(DbContext);
            var result = task.DoTask(testPublisher);

            Assert.IsTrue(result.Success);
            Assert.IsNull(result.Exception);

            var publisherDocuments = result.Data;
            Assert.IsNotNull(publisherDocuments);
            Assert.IsTrue(publisherDocuments.Count >= 2);

            var storageItem1 = publisherDocuments.SingleOrDefault(dm => dm.Uuid == storageItem1Id);
            ...
            var storageItem2 = publisherDocuments.SingleOrDefault(dm => dm.Uuid == storageItem2Id);
        ...
        {
            var task = new ListPublisherDocuments(EmptyDbContext);
